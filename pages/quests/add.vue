...
const frequencies = ref<Record<Frequency, string>>({NONE : 'Unique' , DAILY : 'Journalière' , WEEKLY : 'Hebdomadaire' , MONTHLY : 'Mensuelle'})
...
watch(selectedFrequency, () => {
    console.log(selectedFrequency.value)
    console.log(repeatOrNot.value)
}, {immediate : true})
...
const validateFormQuest = async () => {
    if(title.value !== '' && description.value !== '' && selectedFrequency.value !== null){
        try{
            authStore.loadToken()
